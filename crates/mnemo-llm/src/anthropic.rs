...
impl AnthropicProvider {
    ...
    async fn message(&self, system: &str, user_msg: &str) -> LlmResult<String> {
        ...
        let api_key = self.config.api_key.as_deref().ok_or_else(|| MnemoError::LlmProvider {
        ...
        if response.status() == 429 {
            let retry_after = response.headers()
                ...
                .unwrap_or(5);
            return Err(MnemoError::RateLimited { retry_after_ms: retry_after * 1000 });
        }
...
impl LlmProvider for AnthropicProvider {
    async fn extract_entities_and_relationships(
        ...
        Ok(ExtractionResult {
            entities: parsed.entities.into_iter().map(|e| ExtractedEntity {
                name: e.name,
                entity_type: EntityType::from_str_flexible(&e.entity_type),
                summary: e.summary,
            }).collect(),
            relationships: parsed.relationships.into_iter().map(|r| ExtractedRelationship {
                source_name: r.source, target_name: r.target,
                label: r.label, fact: r.fact,
                confidence: r.confidence, valid_at: None,
            }).collect(),
        })
    }
    ...
    async fn detect_contradictions(&self, new_fact: &str, existing_facts: &[String]) -> LlmResult<Vec<String>> {
        if existing_facts.is_empty() { return Ok(Vec::new()); }
        let existing = existing_facts.iter().enumerate()
            .map(|(i, f)| format!("{}. {}", i + 1, f)).collect::<Vec<_>>().join("\n");
        let system = "You detect contradictions between facts. Respond with a JSON array of strings. If none, respond with [].";
        let user_msg = format!("New fact: {}\n\nExisting facts:\n{}\n\nList contradictions as JSON array.", new_fact, existing);
