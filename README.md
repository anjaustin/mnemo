...
- **Mnemo OpenAPI paths**: All 142 REST handlers have `#[utoipa::path]` annotations with method, path, tag, summary, request/response schemas, and security requirements. Swagger UI shows full endpoint documentation across all 20 tags. Some handlers return `Json<serde_json::Value>` with `Object` response schemas — these are intentionally dynamic (e.g., agent identity core, graph subgraph responses) rather than typed DTOs.
...
- **Mnemo internal types in OpenAPI**: Cleaned up — removed `IngestWebhookEvent`, `ListSessionsParams`, and `ListEpisodesParams` (purely internal). Added `PaginationParams` (the type actually used by HTTP handlers). `signing_secret` hidden from `MemoryWebhookSubscription` schema via `#[schema(ignore)]`. Remaining schemas are all used in API responses.
