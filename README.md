...
- **Mnemo OpenAPI paths**: All 142 REST handlers now have `#[utoipa::path]` annotations with method, path, tag, summary, request/response schemas, and security requirements. Swagger UI shows full endpoint documentation across all 20 tags. Handlers returning `Json<serde_json::Value>` use `Object` response bodies (typed DTOs deferred).
